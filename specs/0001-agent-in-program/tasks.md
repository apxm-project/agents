...
- [X] T063 [P] [US5] `skill_search()` helper + `skills=True` wires the real `search_skills` group into the turn — `proxy.py`, `agent.py`. (`resolve_ask_tools` now merges explicit Python tools with `tool_groups`, so `tools=[lookup]` no longer hides the `skills` group; `skill_search(query=...)` lowers to the live capability's required `request` field; live session `live-sc005-search-import-272e3f5e` called `search_skills` and returned `prompt-as-workflow`.)
...
representative guard; still deferred: full quickstart acceptance on both hosts.
