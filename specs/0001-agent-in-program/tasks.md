...
- [X] T063 [P] [US5] `skill_search()` helper + `skills=True` wires the real `search_skills` group into the turn — `proxy.py`, `agent.py`. (`resolve_ask_tools` now merges explicit Python tools with `tool_groups`, so `tools=[lookup]` no longer hides the `skills` group; `skill_search(query=...)` lowers to the live capability's required `request` field; `search_skills` and the `skills`/`authoring` groups are exported constants/enums; runtime tool dispatch injects execution-scoped visible imports when an agent omits the `imports` arg. Live session `live-sc005-search-import-272e3f5e` called `search_skills` and returned `prompt-as-workflow`; patched live probe `live-sc005-import-inherit-debug` returned `architecture_review` from launch imports without explicit node imports.)
...
- Polish T070, T071 (dead-surface sweep), T073 (docs), T072 offline portion plus
  scoped live `search_skills` evidence.
...
`prompt-as-workflow`. The patched execution path also injects launch imports
into `search_skills` when the agent omits explicit imports: direct live
capability campaign on `127.0.0.1:18918` returned 10/10, graph execution session
`live-sc005-import-inherit-debug` returned `architecture_review`, and Host A CLI
