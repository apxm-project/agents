...
Reference-host wire and Port Contracts are authored and digest-bound by
Agents. No downstream owner descriptor or SDK checkout is required. The
semantic digest identifies the canonical Agents descriptor content without
its self-digest field; the exact checksum detects any byte-level descriptor
drift.
...
verifies every recorded content-addressed digest is current, keeps the
reference-host wire cohort local to Agents, rejects an active downstream SDK
build dependency, and confirms the
