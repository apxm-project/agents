...
The graph JSON contract is simple and stable (see [contracts](../implementation/internals/contracts.md)): nodes with typed operations, edges with dependency kinds (`Data`, `Control`, `Effect`), parameters, and metadata. Any tool that can emit this JSON can target A-PXM.
...
A-PXM's [compiler](../implementation/compiler/overview.md) is built on MLIR. Optimization is a pipeline of independent passes documented in [optimization/passes.md](../optimization/passes.md). Contributors can add new passes without understanding the full compiler. Over time, this pass library compounds -- every new optimization benefits every agent built on A-PXM, past and future.
...
**The optimization moat.** Every compiler pass benefits every agent. New passes -- CondenseOps, profile-guided tier adaptation, cross-agent fusion -- compound on top of existing ones. See [optimization/passes.md](../optimization/passes.md) for the current inventory. An ad-hoc runtime for a single agent cannot accumulate this library.
...
The [AAM](aam.md) defines agent state as `(B, G, C)`. The [hierarchical AAM design](../design/hierarchical-aam.md) shows that a directory tree is a natural implementation:
...
A-PXM's current implementation provides the [foundations](foundations.md): typed AIS operations, MLIR-based compilation, token-counting dataflow scheduling, three-tier memory, and the AAM state model. See the [strategy](../strategy/) directory for detailed roadmap.
...
- [Hierarchical AAM](../design/hierarchical-aam.md) -- the file tree as AAM implementation
- [Optimization Passes](../optimization/passes.md) -- compiler pass inventory
- [Strategy](../strategy/) -- roadmap and gap analysis
