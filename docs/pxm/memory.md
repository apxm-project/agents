...
The three-tier memory hierarchy is not just an architectural abstraction -- it maps directly to how LLM-based agents actually consume and produce context during execution.

### STM as Fast Memory

Short-term memory is the agent's **current conversational stack**. When an LLM-based agent needs to reason about a task, the most critical factor is having the right data in context at the right time. STM serves this role: it holds the immediate working set -- recent tool outputs, intermediate reasoning results, and session-scoped variables -- so that every LLM call receives precisely the context it needs without retrieving irrelevant history. This is the file-tree argument applied to agents: giving the model structured, navigable, in-scope data rather than dumping everything into a flat prompt.

### LTM as Accumulated Knowledge

Long-term memory captures what the agent has learned across sessions -- user preferences, project facts, domain knowledge, cached tool results. This is the durable substrate that prevents an agent from re-discovering the same information on every invocation. LTM persists through SQLite with semantic retrieval (BM25 + vector + MMR), which means the agent can query its own accumulated knowledge in natural language rather than by exact key.

### Episodic Memory as Execution History

The append-only episodic log records what the agent has done: which operations fired, what inputs they received, what outputs they produced, and in what order. This is the substrate for self-reflection. An agent (or a human auditor) can query the episodic log to understand why a particular decision was made, detect loops or regressions, and adapt strategy based on past performance.

### Beliefs, Goals, Capabilities

The combination of these three tiers provides the **Beliefs** component of the Agent Abstract Machine (AAM). STM holds current beliefs about the immediate task. LTM holds durable beliefs about the world. Episodic memory holds beliefs derived from experience. Together with Goals (what the agent is trying to achieve) and Capabilities (what operations it can perform), the AAM's full state is defined.

### Navigability by LLMs

Because APXM's memory is structured as typed key-value stores and queryable logs -- not opaque embeddings or flat text blobs -- it is inherently navigable by LLMs. An agent can read its own STM to understand its current state, query LTM for relevant knowledge, scan episodic logs to review past decisions, and write updated beliefs back through UMEM. External tools and human operators can do the same: inspect, query, and debug the agent's memory state through the same QMEM/UMEM interface that the agent itself uses. Memory is not a black box; it is a file-tree-like hierarchical state that both the agent and its operators can traverse.
