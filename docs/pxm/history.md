...
A-PXM provides the missing compiler. Agent workflows are expressed as AIS dataflow graphs. The compiler, built on MLIR, performs typed analysis and optimization. The current pass pipeline is documented in [optimization/passes.md](../optimization/passes.md).
