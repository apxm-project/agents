...
- **`agent.toml`** — the one manifest an author writes. Every field it can
  ...
  tolerated extra.
- **`integrity.toml`** — generated by `apxm build`. Nobody authors it,
...
- **Widening**, enforced at `apxm build`. A `[permissions]`
...
is what `apxm build` computes and writes into `integrity.toml`'s
chain (§7), keyed by exactly the same recognized-path patterns.

## 7. Integrity: what it is, when to rebuild

`apxm build` hashes every recognized file (SHA-256 of its bytes,
...
last build. **Rebuild (`apxm build`) whenever you edit any recognized
