...
- Measured: 2-10x on realistic graphs (from FINDINGS.md §2)
...
- Measured: 1.51x speedup on chained_llm benchmark (VLLM-RESULTS.md)
...
**Current implementation** (from FINDINGS.md):
...
- **Measured**: 1.51x on shared_prefix_fanout.apxm (FINDINGS.md §1)
...
**Traditional DSPy** (from DSPY-INTEGRATION.md):
...
- **Speedup**: 1.51x → 1.8x (from VLLM-RESULTS.md baseline)

### 4.2 KV Pinning for Downstream Feeding

**What it is** (from VLLM-GRAPH-AWARENESS.md):
...
From existing measurements (FINDINGS.md, VLLM-RESULTS.md):

| Benchmark | Graph Structure | O0 Time | O2+Latency Time | Speedup | Source |
|-----------|----------------|---------|------------------|---------|--------|
| **shared_prefix_fanout** | 4-way fan-out, shared context | 5.39s | 3.58s | **1.51x** | FINDINGS.md §1 |
| **dead_context_stress** | 5 unused context ops | 11.99s | 6.12s | **2.00x** | FINDINGS.md §2.1 |
| **cse_stress** | 3 duplicate prompts | 13.96s | 12.21s | **1.15x** | FINDINGS.md §2.3 |
| **mixed_priority** | Critical + background | 4.23s | 3.79s | **1.12x** | FINDINGS.md §5 |
...
1. **APXM Documentation**
   - docs/implementation/compiler/optimization-passes.md — Current pass pipeline
   - docs/strategy/08-OPTIMIZATION-TARGETS.md — Multi-objective optimization framework
   - docs/strategy/10-DSPY-INTEGRATION.md — DSPy for prompt optimization
   - docs/guides/vllm-integration.md — vLLM graph-aware scheduling

2. **Benchmarks**
   - docs/FINDINGS.md — Comprehensive optimization results (April 7-8, 2026)
   - docs/benchmarks/VLLM-RESULTS.md — vendor GPU + vLLM prefix caching (1.51x speedup)
   - docs/benchmarks/CACHE-RESULTS.md — MemoCache effectiveness (9% speedup)
