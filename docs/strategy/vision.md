...
> For formal execution model theory, see [PXM Foundations](../pxm/). For architecture decisions, see [Design](../design/).
...
> See also: [Opportunities](opportunities.md) for detailed implementation proposals, [Gap Analysis](gap-analysis.md) for current vs target state, [Master Plan](master-plan.md) for validation and timeline, [PXM theory](../pxm/) for formal execution model, [Design](../design/) for architecture decisions.
