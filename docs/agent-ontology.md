...
1. **Primitive ops** — the 40 AIS operations (INV, EXC, QMEM, THINK, etc.)
...
This is where it gets interesting: **a skill IS a graph**.

---

## Part 2: Skills as Graphs

A skill is not a function. It's a compiled APXM subgraph with metadata:
...
4. **Optimization** — the compiler can fuse, parallelize, or reorder skill invocations

## Part 3: Hierarchical AAM and Context Isolation

This is the key insight you identified: **each skill invocation gets its own scoped AAM**.

When a skill is invoked via FLOW_CALL or DELEGATE, the runtime creates a **child scope**:
...
**This is why skills have their own context.**
...
The skill runs in isolation. It can't see the parent's other beliefs. It can't modify them. It returns a result, and the parent decides what to do with it.

---

### Why This Matters: Context Window Management

LLMs have finite context windows. If the agent is a flat (B, G, C), every operation sees everything — the context explodes.
...
Parent Agent (B_parent, G_parent, C_parent)
    ...
    │               │
    │               └── Starts fresh
    │               └── Can't see code_review results unless passed explicitly
```

**The per-node workspace (`nodes/{id}_{name}/`) is the physical manifestation of this.**
...
Same agent, same skill, different context based on profile.
...
3. B executes its skillgraph
4. B returns a result
5. A receives the result into its own B

**Multi-agent = hierarchical AAM with cross-agent FLOW_CALL.**
...
│  ┌─────────────────────────────────────────────────────────┐   │
│  │ CAPABILITIES (C)                                         │   │
│  │  ├── Primitive Ops (40 AIS operations)                   │   │
...
Agent := (B, G, C, S)
where:
  B = Beliefs    : STM × LTM × Episodic
  G = Goals      : PriorityQueue<Goal> with GoalTree
  C = Capabilities : PrimitiveOps ∪ Skills
...
Problem: Agents can interfere with each other, leak context, cause side effects.
...
Solution: Skills declare preconditions and effects. The compiler can:
...
Solution: Per-node workspaces. Each skill invocation has its own trace. Episodic memory is append-only. REFLECT can analyze its own history.
...
APXM: Every reasoning op (THINK, REASON, REFLECT) produces a trace. Every action (INV, EXC) updates beliefs. The interleaving is structural in the graph.

### Meta Semi-Formal Reasoning (2603.01896)

Key insight: Structure prevents skipping cases or making unsupported claims.

APXM: The typed instruction set IS the structure. VERIFY requires evidence. WAIT_ALL requires all upstream tokens. The scheduler enforces it.
...
The agent is not a skillgraph. The agent HAS skillgraphs as its capabilities. The agent IS the (B, G, C) triple that provides context and goals for those skills to execute in.

---

*This document is the conceptual foundation of APXM.*
