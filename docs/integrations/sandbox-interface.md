...
1. **apxm owns the interface; backends are swappable backings.** The trait lives in `apxm-runtime/src/sandbox/`. Process / Bubblewrap / OpenShell / Firecracker / gVisor / Docker are implementations registered by the host. No vendor type appears on any core struct; backend-specific config rides `SessionPolicy.extra` (opaque `serde_json::Value`).
...
### 2.7 SessionPolicy (replaces legacy `policy.rs::SandboxPolicy`)
...
- **Bubblewrap (Container, Linux-only, non-persistent, kvm=false):** `is_available` MEMOIZED (`OnceLock<bool>` over `bwrap --version` — fixes the per-query fork at sandbox_linux.rs:215). `create_session` allocates TempDir into inner; reads `cfg.policy`. `execute` forks fresh bwrap. `validate` → `Degraded { gaps: [read_paths(per-path), resource_limits.*(no cgroups)] }`. Implements `spawn` for ACP (agent as long-lived bwrap entry process with piped stdio) — currently absent, a migration item. Migrate its legacy `SandboxPolicy` env-filter field to `cfg.policy.credentials`. `destroy_session` = no-op (TempDir Drop).
...
- **Breaking renames (mechanical):** `ValidationResult::Degraded { warnings: Vec<String> }` → `{ gaps: Vec<CapabilityGap> }` (compile error at sandbox_linux.rs:78-87, capability/mod.rs:295, registry.rs:443, backend.rs:239 until updated); legacy `policy.rs::SandboxPolicy` → `SessionPolicy`.
...
1. Step 1 (runtime types, mechanical rename): in apxm-runtime/src/sandbox/backend.rs change ValidationResult::Degraded { warnings: Vec<String> } to { gaps: Vec<CapabilityGap> }; add CapabilityGap + GapSeverity. Add SandboxError::Unsupported. Fix the only in-crate constructors: registry.rs:443 test double and backend.rs:239 test. This is the first commit so everything downstream compiles against the new shape.
...
6. Step 6 (Process + Bubblewrap backends): migrate sandbox_linux.rs and process.rs to read SessionConfig.policy; memoize Bubblewrap::is_available with OnceLock<bool>; convert validate to emit Degraded { gaps } with named fields + GapSeverity; migrate bwrap's legacy SandboxPolicy env-filter field to cfg.policy.credentials; add supported_platforms (bwrap=Linux, Process=all). Implement bwrap spawn() for ACP (agent as long-lived bwrap entry, piped stdio). Run the sandbox unit + integration tests.
