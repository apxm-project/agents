# Host Integration -- Codex & Gemini CLI
...
> -- [PXM Foundations](../pxm/foundations.md)
...
## 5. AAM State Separation

The AAM (Agent Abstract Machine) separates state from compute so the compiler can optimize, the runtime can checkpoint, and hosts can scope multi-agent delegation. For the formal definition and full API, see:

- [Agent Abstract Machine (AAM)](../pxm/aam.md) -- formal `(B, G, C)` triple, transition function, memory hierarchy
- [Hierarchical AAM](../design/hierarchical-aam.md) -- scoping rules, file-tree-as-AAM design, implementation diagrams
- [Sessions](runtime/sessions.md) -- `AamCheckpoint`, `SessionManager`, `ScopeSpec`, and `ProcessTable` implementation
