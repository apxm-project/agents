...
6. Structural conformance (required fields, node-kind enum, edge-kind enum, acyclic graph) is `apxm.workflow-draft.v1` territory — trust `auditWorkflowDraft`'s schema-generated checks (G-5) over eyeballing the JSON; call out a violation by naming the contract rule it breaks, not by restating the enum from memory.
