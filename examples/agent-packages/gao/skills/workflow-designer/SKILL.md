...
instance of the canonical `apxm.workflow-draft.v1` contract (WF-1) — the same
schema Studio's own canvas persistence validates against and `auditWorkflowDraft`
checks Gao drafts with (G-5). This skill does not restate that schema by hand;
...
`examples/*.air` are the WF-2/WF-3 vector-set fixtures
