...
- if a turn needs a **write** capability, APXM must mint a capability grant
  for the requested tool binding and retry the turn with the returned `grant_*`
  id in `capability_grant_ids`.
