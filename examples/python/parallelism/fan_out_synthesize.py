...
def fan_out_synthesize(g: GraphRecorder):
    ...
    plan = g.ask(
        name="plan",
        prompt="Create a 3-part outline for a blog post about Rust async programming. "
        "Format as:\nPART 1: [title]\nPART 2: [title]\nPART 3: [title]"
    )

    # PARALLEL: these 3 nodes share no data deps between them --
    # the compiler schedules them concurrently. Bind each result to a
    # local matching the explicit name so the auto-wire below resolves.
    section_concepts = g.ask(
        name="section_concepts",
    ...
    assemble = g.think(
        name="assemble",
        prompt="Assemble these independently-written sections into a polished blog post:\n\n"
        "SECTION 1:\n{section_concepts}\n\n"
        "SECTION 2:\n{section_tokio}\n\n"
        "SECTION 3:\n{section_pitfalls}\n\n"
    ...
    output = g.print(message="=== BLOG POST ===\n{assemble}")
    g.done(output)
...
if __name__ == "__main__":
    ...
    result = apxm.run(fan_out_synthesize())
