...
class BackendType(str, Enum):
    """APXM backend type values used by `dekk agents backend add`."""
...
class BackendProtocol(str, Enum):
    """APXM backend protocol values used by `dekk agents backend add`."""
...
class VllmCommand(str, Enum):
    """Subcommands exposed under `dekk agents vllm`."""
...
class VllmDefaults:
    ...
    # at crates/runtime/backends/src/llm/backends/openai/backend.rs:779-789 can populate
...
@dataclass(frozen=True)
class ProbeContract:
    """Temporary graph ids and node names used by `dekk agents vllm probe`."""
