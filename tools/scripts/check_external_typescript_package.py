...
def validate_frontend_graph(graph: dict[str, Any]) -> None:
    ...
    if graph.get("schema_version") != "apxm.frontend-graph.v1":
        fail("external package did not emit apxm.frontend-graph.v1")
...
def validate_air(air: dict[str, Any]) -> None:
    ...
    if air.get("schema_version") != "apxm.air.v1":
        fail("external package did not emit apxm.air.v1")
