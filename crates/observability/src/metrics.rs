...
//! - token usage
//!
//! There is no routing-decision instrument. A `model.call` target resolves to
//! exactly one bound Model Deployment or fails closed, so there is no
//! candidate set, no passed-over candidate, and nothing for a
//! chosen/rejected outcome label to describe.
//!
