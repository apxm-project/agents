...
fn set_env_if_missing(key: &str, value: &Path) {
    if env::var_os(key).is_none() {
        // SAFETY: Called during single-threaded env setup before any threads are spawned.
        #[allow(unsafe_code)]
        unsafe {
...
pub fn get_workspace_root(manifest_dir: &Path) -> Result<PathBuf> {
    // Tiered layout: crates/{tier}/apxm-{name}/ → 3 parent() calls to reach workspace root
    manifest_dir
        .parent()
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
