...
mod tests {
    ...
    async fn test_default_backend_lifecycle() {
        let backend = DefaultBackend::new(test_capabilities(), |_req| async {
            Ok(ExecResult {
                success: true,
                exit_code: Some(0),
    ...
    fn test_validation_result_variants() {
        ...
        let degraded = ValidationResult::Degraded {
            warnings: vec!["no network restriction".into()],
        };
        assert!(matches!(degraded, ValidationResult::Degraded { .. }));

        let unsupported = ValidationResult::Unsupported {
            reason: "bwrap not found".into(),
        };
        assert!(matches!(unsupported, ValidationResult::Unsupported { .. }));
    }
}
