...
mod tests {
    ...
    fn resolves_spawn_agent_profile_skill() {
        ...
        let skills_root = dir.path().join(".agents/skills/claude");
        fs::create_dir_all(&skills_root).expect("skills dir");
        fs::write(skills_root.join("SKILL.md"), "# claude").expect("skill file");
        ...
        assert_eq!(resolver.skill_name(&resolved[0]), "claude");
    }
}
