...
mod tests {
    ...
    #[test]
    fn matches_direct_apxm_run() {
        assert!(matches_direct_apxm_job(&args(&[
            "/repo/target/release/apxm",
            "run",
            "graph.apxmobj",
        ])));
    }
    ...
    #[test]
    fn matches_dekk_apxm_execute_through_python_wrapper() {
    ...
    #[test]
    fn matches_workflow_run_but_not_process_stop() {
        assert!(matches_direct_apxm_job(&args(&[
            "apxm",
            "workflow",
            "run",
    ...
    #[test]
    fn services_and_vllm_are_opt_in() {
        let default_scope = ProcessScope {
            include_services: false,
            include_vllm: false,
        };
        let full_scope = ProcessScope {
            include_services: true,
            include_vllm: true,
        };
        assert!(classify_process(&args(&["apxm-server"]), &default_scope).is_none());
        assert_eq!(
            classify_process(&args(&["apxm-server"]), &full_scope),
            Some("apxm-service".to_string())
        );
        assert!(
            classify_process(
    ...
    #[test]
    fn resolves_project_root_from_nested_path() {
        let nested = Path::new(env!("CARGO_MANIFEST_DIR")).join("src/commands");
        let root = find_project_root(&nested).expect("project root");
