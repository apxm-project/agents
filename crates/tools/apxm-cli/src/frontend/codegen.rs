...
fn render_operations_module() -> String {
    ...
    buf.push_str(&format!(
        ...
        llm_ops.iter().map(|op| py_string(op)).collect::<Vec<_>>().join(", "),
    ));
...
fn render_emission_module() -> String {
    ...
    // Helper function for quoting MLIR strings
...
fn render_emission_fn(buf: &mut String, spec: &FrontendEmissionSpec) {
    ...
    if let Some((open, close)) = delims {
        buf.push_str(&format!(
            "    ctx = f\" {open}{{', '.join(inputs)}} : {{', '.join(['!ais.token'] * len(inputs))}}{close}\" if inputs else \"\"\n"
        ));
    } else {
        buf.push_str("    ctx = \"\"\n");
    }
    ...
    if let Some(primary) = &spec.primary_attr {
        // Output: primary = f' {_quote(str(attrs["KEY"]))}' if "KEY" in attrs else ""
        // In Rust format!, {{ → literal { in output, so {{_quote(...)}} → {_quote(...)} in Python
        buf.push_str(&format!(
    ...
    if !spec.keywords.is_empty() {
        ...
        for kw in &spec.keywords {
            ...
            buf.push_str(&format!(
                "        kw_parts.append(f'{}={{attrs[\"{}\"]}}')\n",
                kw, kw
            ));
        }
        // Output Python: kw_str = f' {{{", ".join(kw_parts)}}}' if kw_parts else ""
        // In Python f-string: {{ → literal {, {expr} → evaluated, }} → literal }
    ...
    if spec.result_type == "Void" {
        buf.push_str(&format!(
            "    return f\"ais.{}{{primary}}{{ctx}}{{kw_str}}\"\n\n\n",
            spec.mlir_mnemonic
        ));
    } else {
        ...
        buf.push_str(&format!(
            "    return f\"{{ssa_name}} = ais.{}{{primary}}{{ctx}}{{kw_str}} : {}\"\n\n\n",
...
mod tests {
    ...
    fn emission_module_renders_emitters_dict() {
        let rendered = render_generated_files();
        assert!(rendered.emission_py.contains("EMITTERS: dict[str, Callable]"));
        assert!(rendered.emission_py.contains("VOID_OPS: Final[frozenset]"));
        assert!(rendered.emission_py.contains("TEMPLATE_ATTRS: Final[frozenset]"));
        // Check new function signature
        assert!(rendered.emission_py.contains("def emit_ask(ssa_name: str, attrs: dict[str, Any], inputs: list[str])"));
    }
