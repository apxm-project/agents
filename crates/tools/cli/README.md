...
The shell sequences a Compilation Client (`apxm build`) and an Interaction
Client (`apxm run`, TUI, `apxm event`, `apxm resume`).
...
The CLI exposes no transcript/session command family. OpenAI
`/v1/chat/completions` is an independently owned protocol edge.
