...
fn live_required_cases() -> Vec<String> {
    ...
    manifest["executable_parity_evidence"]["live_required_cases"]
        .as_array()
...
fn unavailable_required_cases() -> Vec<(String, String)> {
    let manifest = release_manifest();
    manifest["executable_parity_evidence"]["unavailable_required_cases"]
        .as_array()
        .expect("unavailable_required_cases array")
        .iter()
        .map(|entry| {
            (
                entry["name"].as_str().expect("unavailable case name").to_owned(),
                entry["reason"]
                    .as_str()
                    .expect("unavailable case reason")
                    .to_owned(),
            )
        })
        .collect()
...
fn materialize_admission(host: &HostProcess, fixture: &Value) -> Value {
    ...
    let provenance_matches_artifact = admission["provenance_digest"] == admission["artifact_digest"];
...
fn materialize_expected_response(host: &HostProcess, expected: &Value) -> Value {
    ...
    if let Some(readiness) = materialized.get_mut("readiness").and_then(Value::as_object_mut) {
        readiness.insert(
            "release_digest".into(),
...
fn assert_no_runtime_evidence(response: &Value) {
    if let Some(runtime_evidence) = response.get("runtime_evidence") {
        assert!(runtime_evidence.is_null(), "runtime_evidence must be null or absent");
    }
}
...
fn assert_case_partition() {
    ...
    let unavailable_cases = unavailable_required_cases();
    let unavailable_names: Vec<String> = unavailable_cases
        .iter()
        .map(|(name, _reason)| name.clone())
        .collect();
    let combined_cases = [live_cases.clone(), unavailable_names.clone()].concat();
    assert_eq!(combined_cases, all_cases, "case partition drifted from the vectors");
    assert_eq!(unavailable_cases.len(), 1, "unexpected unavailable live-case count");
    assert_eq!(
        unavailable_cases[0].0,
        "drain_shutdown_after_in_flight_completion"
    );
    assert!(
        !unavailable_cases[0].1.trim().is_empty(),
        "unavailable live-case reason must be non-empty"
    );
}

#[test]
fn executable_parity_manifest_partitions_live_and_unavailable_cases() {
    ...
    for case_name in executed_cases {
        ...
        match case_name.as_str() {
            "cancellation_before_admission" => {
                ...
                assert_json_subset(
                    &cancelled,
                    &materialize_expected_response(
                        &host,
                        &expected["expected_host_response"],
                    ),
                    "cancelled",
                );
