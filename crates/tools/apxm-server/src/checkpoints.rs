...
mod store_tests {
    ...
    #[test]
    fn resume_is_exactly_once_in_memory() {
        let s = CheckpointStore::new();
        s.create(pending("x"));
        assert!(s.resume("x", serde_json::json!({ "ok": true })).is_ok());
        // Second resume of an already-resumed checkpoint is rejected.
        assert!(s.resume("x", serde_json::json!("again")).is_err());
        // Unknown checkpoint errors.
        assert!(s.resume("nope", serde_json::json!(1)).is_err());
    }

    #[test]
    fn durable_store_survives_reopen_and_is_exactly_once() {
        ...
        let path = dir.join("checkpoints.sqlite");

        // Session 1: create + resume, then drop the store (simulate shutdown).
        {
        ...
        {
            let s = CheckpointStore::open(&path).unwrap();
            let c1 = s.get("dur-1").expect("dur-1 recovered");
            assert_eq!(c1.status, CheckpointStatus::Resumed);
            assert_eq!(c1.human_input, Some(serde_json::json!({ "answer": 42 })));
            let c2 = s.get("dur-2").expect("dur-2 recovered");
            assert_eq!(c2.status, CheckpointStatus::Pending);

            // Exactly-once across restart: a checkpoint resumed before the restart
            // cannot be resumed again.
            assert!(
                s.resume("dur-1", serde_json::json!("again")).is_err(),
                "already-resumed checkpoint must reject a second resume after restart"
            );
            // The still-pending one can be resumed once.
            assert!(s.resume("dur-2", serde_json::json!("ok")).is_ok());
        }
