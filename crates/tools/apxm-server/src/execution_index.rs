...
mod tests {
    ...
    fn make_record(execution_id: &str, session_dir: &Path, skill_id: &str) -> ExecutionRecord {
        ExecutionRecord {
    ...
    #[test]
    fn upsert_writes_sidecar_and_caches() {
        let temp = tempfile::tempdir().expect("tempdir");
        let session_dir = temp.path().join("sessions/skills/x/session-1");
        let record = make_record("exec-1", &session_dir, "skill-a");
        write_snapshot(&record);

        let index = ExecutionIndex::new();
        index.upsert_from_record(&record);

        let sidecar_path = session_dir.join("executions").join(INDEX_FILE_NAME);
        assert!(sidecar_path.is_file(), "sidecar should exist");
        let sidecar = read_sidecar(&session_dir.join("executions")).expect("sidecar parse");
        assert!(sidecar.contains_key("exec-1"));

        let entry = index.get("exec-1").expect("cached entry");
        assert_eq!(entry.skill_id.as_deref(), Some("skill-a"));
    }

    #[test]
    fn lru_evicts_oldest_but_keeps_sidecar() {
        let temp = tempfile::tempdir().expect("tempdir");
        let session_dir = temp.path().join("sessions/skills/x/session-1");
        let index = ExecutionIndex::with_capacity(2);

        for i in 0..3 {
            let id = format!("exec-{i}");
            let record = make_record(&id, &session_dir, "skill-a");
            write_snapshot(&record);
            index.upsert_from_record(&record);
        }

        assert_eq!(index.in_memory_len(), 2);
        // exec-0 was evicted from memory but is still on disk + in sidecar.
        let resolved = index.get("exec-0").expect("disk probe rehydrates");
        assert_eq!(resolved.skill_id.as_deref(), Some("skill-a"));
    }

    #[test]
    fn reload_rebuilds_from_directory_when_sidecar_missing() {
        let temp = tempfile::tempdir().expect("tempdir");
        let session_root = temp.path().join("sessions");
        let session_dir = session_root.join("skills/x/session-1");
        for i in 0..3 {
            let record = make_record(&format!("exec-{i}"), &session_dir, "skill-a");
            write_snapshot(&record);
        }
        // No sidecar yet.
        let index = ExecutionIndex::new();
        let loaded = index.reload_from_session_roots([session_root.clone()]);
        assert_eq!(loaded, 3);
        assert!(
            session_dir
                .join("executions")
                ...
                .is_file(),
            "reload should write the sidecar"
        );
    }

    #[test]
    fn reload_directory_wins_against_stale_sidecar() {
        let temp = tempfile::tempdir().expect("tempdir");
        let session_root = temp.path().join("sessions");
        let session_dir = session_root.join("skills/x/session-1");
        let record = make_record("exec-0", &session_dir, "skill-a");
        write_snapshot(&record);

        // Hand-craft a sidecar that mentions an id whose file does not exist
        // and omits the id that does exist.
        let executions_dir = session_dir.join("executions");
        let mut sidecar: HashMap<String, SidecarEntry> = HashMap::new();
        sidecar.insert(
            "ghost-id".to_string(),
            SidecarEntry {
                skill_id: Some("ghost".to_string()),
                started_at_ms: 42,
                finished_at_ms: None,
                status: ExecutionStatus::Running,
            },
        );
        write_sidecar(&executions_dir, &sidecar);

        let index = ExecutionIndex::new();
        let loaded = index.reload_from_session_roots([session_root]);
        assert_eq!(loaded, 1);
        assert!(index.get("exec-0").is_some());
        assert!(index.get("ghost-id").is_none(), "ghost must not survive");

        let rewritten = read_sidecar(&executions_dir).expect("sidecar");
        assert!(rewritten.contains_key("exec-0"));
        assert!(!rewritten.contains_key("ghost-id"));
    }
}
