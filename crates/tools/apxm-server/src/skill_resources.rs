...
// Shared by the sibling `apxm_mcp` binary (which #[path]-includes this module)
// and in-crate tests; invisible to the `apxm-server` binary's dead_code lint.
#[allow(dead_code)]
...
// Helper for `scan_resource_packages`; reachable only through the shared
// `apxm_mcp` and test entry point.
#[allow(dead_code)]
