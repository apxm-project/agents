...
export function programInstanceReceiver(instance: ProgramInstanceRef): Record<string, string> {
...
export function programNewOperands(spec: ProgramNewSpec): Record<string, unknown> {
  ...
  if (spec.initial_context !== undefined) {
    operands.initial_context = spec.initial_context;
  }
  return operands;
}
