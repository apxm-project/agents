...
/// A generic Agent Program submitted from outside the compiler crate compiles
/// through the ordinary source boundary. The fixture carries no product name,
/// product-owned identifier, or product-specific source semantics.
#[test]
fn a_generic_external_program_compiles_as_ordinary_submitted_source() {
    ...
    let compiled = compile_source_bundle(
        &SourceBundleRequest::new(Frontend::Typescript, "ExternalFixture", source),
        &roots(),
        &drivers(),
    )
    .expect("generic external source compiles through the ordinary boundary");
