...
mod tests {
    ...
    const TEST_BATCH_GROUP: &str = "review";
    const UNSUPPORTED_LATENCY_CLASS: &str = "fast";

    #[test]
    fn latency_class_serializes_as_typed_snake_case() {
        assert_eq!(
            serde_json::to_value(LatencyClass::Short).unwrap(),
            serde_json::Value::String(LatencyClass::Short.as_str().to_owned())
        );
        assert_eq!(
            LatencyClass::from_str(LatencyClass::Long.as_str()).unwrap(),
            LatencyClass::Long
        );
        assert!(LatencyClass::from_str(UNSUPPORTED_LATENCY_CLASS).is_err());
    }

    #[test]
    fn node_graph_metrics_loads_from_graph_attrs() {
        let attrs_map = NodeGraphMetrics::default()
            .with_fanout_count(3)
            .with_remaining_path_len(5)
            .with_latency_class(LatencyClass::Medium)
            .with_batch_group(TEST_BATCH_GROUP)
            .with_stage_index(2)
            .with_estimated_dynamic_tokens(128)
            .to_attrs();

        let metrics = NodeGraphMetrics::from_attrs(&attrs_map);

        assert_eq!(metrics.fanout_count, Some(3));
        assert_eq!(metrics.remaining_path_len, Some(5));
        assert_eq!(metrics.latency_class, Some(LatencyClass::Medium));
        assert_eq!(metrics.batch_group.as_deref(), Some(TEST_BATCH_GROUP));
        assert_eq!(metrics.stage_index, Some(2));
        assert_eq!(metrics.estimated_dynamic_tokens, Some(128));
    }

    #[test]
    fn node_graph_metrics_rejects_invalid_latency_class() {
        let attrs_map = HashMap::from([(
            attrs::LATENCY_CLASS.to_owned(),
            Value::String(UNSUPPORTED_LATENCY_CLASS.to_owned()),
        )]);

        assert!(NodeGraphMetrics::try_from_attrs(&attrs_map).is_err());
    }
}
