...
mod tests {
    ...
    #[test]
    fn from_task_dag_preserves_and_lowers() {
        let mut task_dag = TaskDag::new("research.main")
            .add_task(Task::new(1, "research", "Gather references"))
            .add_task(Task::new(2, "write", "Write summary").add_dependency(1));
        task_dag.metadata.parameters = vec![FlowParameter {
            name: "topic".to_string(),
            type_name: "str".to_string(),
        }];

        let flow = AgentFlow::from_task_dag("research.main", task_dag, true)
            .expect("task DAG should lower");

        assert_eq!(flow.name, "research.main");
        assert!(flow.is_entry);
        assert!(flow.task_dag.is_some());
        assert_eq!(
