...
mod tests {
    ...
    #[test]
    fn observed_graph_uses_runtime_durations_and_queue_wait() {
        use crate::types::execution::NodeMetadata;
        use crate::types::operations::AISOperationType;
        use crate::types::{DependencyType, Edge, Node};
        ...
        dag.add_node(Node {
            ...
            attributes: HashMap::new(),
            input_tokens: vec![10],
            output_tokens: vec![30],
            metadata: NodeMetadata::default(),
        })
        .unwrap();
        dag.add_edge(Edge::new(1, 2, 10, DependencyType::Data))
            .unwrap();
        dag.add_edge(Edge::new(1, 3, 10, DependencyType::Data))
            .unwrap();
        ...
        let observed = ObservedGraphMetrics::from_dag_and_statuses(&dag, &statuses)
            .expect("observed graph metrics");
        assert_eq!(observed.critical_path.nodes, vec![1, 2]);
        assert_eq!(observed.critical_path.duration_ms, 30);
        assert_eq!(observed.critical_path.finish_ms, 35);
        assert_eq!(observed.queue_wait.total_ms, 25);
        assert_eq!(observed.queue_wait.critical_path_total_ms, 5);
    }
}
