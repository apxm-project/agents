...
pub(crate) fn model_context_metrics(
    ...
    match plan_metrics {
        ...
        None => ModelContextMetrics::unplanned(node_id, call_kind),
    }
}
...
async fn execute_llm_once(
    ...
    if mode == LlmMode::Ask && request.has_tools() {
        return execute_ask_with_tools_attempt(
            ctx,
            node,
            request,
            attempt,
    ...
    let response = super::execute_llm_request_for_node_with_context_and_generation(
        ctx,
        node,
        mode_name,
        request,
