//! TypeScript handler registry backed by the portable artifact sidecar.
...
/// Registry of TypeScript handlers, keyed by capability name and handler ID.
...
impl TypeScriptHandlerRegistry {
    /// Build a TypeScript registry from the shared cross-language manifest.
    pub fn from_manifest(manifest: HandlerManifest) -> Result<Self, RuntimeError> {
        ...
        for descriptor in manifest
            .handlers
            .into_iter()
            .filter(|descriptor| descriptor.language == HandlerLanguage::TypeScript)
        {
    ...
    /// Number of TypeScript-backed tools.
    pub fn len(&self) -> usize {
        self.tools.len()
    }

    /// Whether no TypeScript-backed tools are registered.
