...
mod tests {
    ...
    fn cap() -> ManageTaskCapability {
        ManageTaskCapability::new(Aam::new(), ToolsStore::in_memory().unwrap())
    }
    ...
    async fn create_then_list_then_complete() {
        ...
        let created = c
            .execute(arg(&[
                ...
                ("description", Value::String("write spec".into())),
            ]))
            .await
            .unwrap();
        let id = task_id_of(&created);

        let listed = c
            .execute(arg(&[("action", Value::String("list".into()))]))
            .await
            .unwrap();
        let tasks = listed
            .as_object()
        ...
        let done = c
            .execute(arg(&[
                ("action", Value::String("complete".into())),
                ("task_id", Value::String(id.clone())),
            ]))
            .await
            .unwrap();
        assert_eq!(
            done.as_object().unwrap().get("status").unwrap().as_str(),
            Some("completed")
        );
        // Durable mirror reflects the completion.
        let row = c
            .store
            .load_tasks()
            .unwrap()
    ...
    #[tokio::test]
    async fn subtask_completion_propagates_to_parent() {
        let c = cap();
        let parent = c
            .execute(arg(&[
                ("action", Value::String("create".into())),
                ("description", Value::String("parent".into())),
                ("completion_policy", Value::String("all_children".into())),
            ]))
            .await
            .unwrap();
        let parent_id = task_id_of(&parent);
        let child = c
            .execute(arg(&[
                ("action", Value::String("create".into())),
                ("description", Value::String("child".into())),
                ("parent_id", Value::String(parent_id.clone())),
            ]))
            .await
            .unwrap();
        let child_id = task_id_of(&child);

        c.execute(arg(&[
            ("action", Value::String("complete".into())),
            ("task_id", Value::String(child_id)),
        ]))
        .await
        .unwrap();
        ...
        let got = c
            .execute(arg(&[
                ...
                ("task_id", Value::String(parent_id)),
            ]))
            .await
            .unwrap();
        assert_eq!(
    ...
    #[tokio::test]
    async fn completion_policy_is_persisted() {
        let c = cap();
        let created = c
            .execute(arg(&[
                ("action", Value::String("create".into())),
                ("description", Value::String("parent".into())),
                ("completion_policy", Value::String("any_child".into())),
            ]))
            .await
            .unwrap();
        let id = task_id_of(&created);
        let row = c
            ...
            .find(|r| r.id == id)
            .unwrap();
        assert_eq!(row.policy.as_deref(), Some("any_child"));
    }

    #[tokio::test]
    async fn update_changes_description_priority_status_and_policy() {
        let c = cap();
        let created = c
            .execute(arg(&[
                ("action", Value::String("create".into())),
                ("description", Value::String("old".into())),
            ]))
            .await
            .unwrap();
        let id = task_id_of(&created);
        ...
        c.execute(arg(&[
            ("action", Value::String("update".into())),
            ("task_id", Value::String(id.clone())),
            ("description", Value::String("new".into())),
            ("priority", Value::Number(Number::Integer(7))),
            ("status", Value::String("active".into())),
            ("completion_policy", Value::String("manual".into())),
        ]))
        .await
        .unwrap();
        ...
        let got = c
            ...
            .await
            .unwrap();
        let got = got.as_object().unwrap();
        assert_eq!(got.get("description").unwrap().as_str(), Some("new"));
        assert_eq!(got.get("priority").unwrap().as_i64(), Some(7));
        assert_eq!(got.get("status").unwrap().as_str(), Some("active"));
        let row = c
            .store
            .load_tasks()
            .unwrap()
            ...
            .find(|r| r.id == id)
            .unwrap();
        assert_eq!(row.policy.as_deref(), Some("manual"));
    }

    #[tokio::test]
    async fn update_requires_a_mutation() {
        let c = cap();
        let created = c
            .execute(arg(&[
                ("action", Value::String("create".into())),
    ...
    #[tokio::test]
    async fn invalid_completion_policy_errors() {
        let c = cap();
        let err = c
            .execute(arg(&[
                ("action", Value::String("create".into())),
                ("description", Value::String("parent".into())),
                ("completion_policy", Value::String("eventually".into())),
            ]))
            .await
            .unwrap_err();
        assert!(format!("{err}").contains("completion_policy"));
    }
    ...
    async fn unknown_task_errors() {
        ...
        let err = c
            .execute(arg(&[
                ("action", Value::String("get".into())),
                ("task_id", Value::String("nope".into())),
            ]))
            .await
            .unwrap_err();
