...
mod tests {
    ...
    fn json_dot_path_navigates_objects_and_arrays() {
        ...
        assert_eq!(
            json_dot_path(&v, "data.0.x"),
            Some(&JsonValue::String("y".into()))
        );
    ...
    async fn rest_mode_fills_url_template_and_builds_body() {
        ...
        let mut cap = ProviderCallCapability::named_rest(
            "provider.media_create",
            "create",
            json!({}),
            "POST",
            "https://api.provider.test/v1/{account_id}/media",
        );
        ...
        assert!(
            ...
            "credential must never be in body"
        );
    }
    ...
    async fn result_path_extracts_field() {
        ...
        assert_eq!(
            out,
            Value::String("17841".into()),
            "should return just the id"
        );
    }
