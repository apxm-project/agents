...
mod tests {
    ...
    fn sched(id: &str, next: i64) -> ScheduleRow {
        ScheduleRow {
            id: id.into(),
            kind: "once".into(),
            every_secs: None,
    ...
    #[test]
    fn schedule_upsert_get_and_due() {
        let s = ToolsStore::in_memory().unwrap();
        s.upsert_schedule(&sched("a", 100)).unwrap();
        s.upsert_schedule(&sched("b", 300)).unwrap();
        assert_eq!(s.get_schedule("a").unwrap().unwrap().next_fire_ms, 100);
        assert_eq!(s.next_armed_fire_ms().unwrap(), Some(100));
    ...
    #[test]
    fn one_shot_fire_marks_fired_recurring_rearms() {
        let s = ToolsStore::in_memory().unwrap();
        s.upsert_schedule(&sched("once", 100)).unwrap();
        s.record_fire("once", 150, None).unwrap();
        assert_eq!(s.get_schedule("once").unwrap().unwrap().status, "fired");
        assert!(s.armed_schedules().unwrap().is_empty());

        let mut rec = sched("rec", 100);
        rec.kind = "recurring".into();
        rec.recurring = true;
        rec.every_secs = Some(60);
        s.upsert_schedule(&rec).unwrap();
        s.record_fire("rec", 150, Some(60_150)).unwrap();
        let row = s.get_schedule("rec").unwrap().unwrap();
        assert_eq!(row.status, "armed");
        assert_eq!(row.next_fire_ms, 60_150);
    }

    #[test]
    fn cancel_only_affects_armed() {
        let s = ToolsStore::in_memory().unwrap();
        s.upsert_schedule(&sched("a", 100)).unwrap();
        assert!(s.cancel_schedule("a").unwrap());
        assert!(!s.cancel_schedule("a").unwrap());
        assert_eq!(s.get_schedule("a").unwrap().unwrap().status, "cancelled");
    }

    #[test]
    fn task_upsert_load_delete() {
        let s = ToolsStore::in_memory().unwrap();
        let row = TaskRow {
            id: "t1".into(),
            parent_id: None,
        ...
        s.upsert_task(&updated).unwrap();
        assert_eq!(s.load_tasks().unwrap()[0].status, "Completed");
    ...
    fn durable_across_reopen() {
        let dir = std::env::temp_dir().join(format!("apxm-toolstore-{}", now_ms()));
        let path = dir.join(apxm_core::constants::agent_tools::STORE_FILENAME);
        {
            let s = ToolsStore::open(&path).unwrap();
            s.upsert_schedule(&sched("keep", 999)).unwrap();
        }
        {
            let s = ToolsStore::open(&path).unwrap();
            assert_eq!(s.get_schedule("keep").unwrap().unwrap().next_fire_ms, 999);
        }
