...
mod tests {
    ...
    #[test]
    fn safe_workflow_filename_accepts_simple_names_and_appends_ext() {
        assert_eq!(
            safe_workflow_filename("c", "greeter").unwrap(),
            "greeter.air"
        );
        assert_eq!(
            safe_workflow_filename("c", "my.flow.air").unwrap(),
            "my.flow.air"
        );
    }

    #[test]
    fn safe_workflow_filename_rejects_traversal_and_separators() {
        assert!(safe_workflow_filename("c", "../etc/passwd").is_err());
        assert!(safe_workflow_filename("c", "a/b").is_err());
        assert!(safe_workflow_filename("c", "..").is_err());
        assert!(safe_workflow_filename("c", "").is_err());
    }

    #[tokio::test]
    async fn compose_rejects_non_air_text() {
        let cap = ComposeWorkflowCapability::new();
        let mut args = HashMap::new();
        args.insert("name".to_string(), Value::String("x".to_string()));
        args.insert("air".to_string(), Value::String("not air".to_string()));
        assert!(cap.execute(args).await.is_err());
    }

    #[test]
    fn authoring_capabilities_are_not_read_only() {
        assert!(!ComposeWorkflowCapability::new().metadata().read_only);
        assert!(!RunWorkflowCapability::new().metadata().read_only);
        assert!(
            ComposeWorkflowCapability::new()
                .metadata()
                .groups
                .iter()
                .any(|g| g == AUTHORING_GROUP)
        );
    }
}
