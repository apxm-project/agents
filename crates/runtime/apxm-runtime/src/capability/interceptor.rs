...
mod tests {
    ...
    fn gate(strict: bool) -> PermissionInterceptor {
        PermissionInterceptor::new(HashSet::from(["slack.post".to_string()]), strict)
    }

    #[tokio::test]
    async fn strict_denies_auth_tool_without_credential() {
        let decision = gate(true).pre_invoke("slack.post", &HashMap::new()).await;
        assert!(matches!(decision, InterceptDecision::Deny { .. }));
    }

    #[tokio::test]
    async fn advisory_allows_but_does_not_deny() {
        let decision = gate(false).pre_invoke("slack.post", &HashMap::new()).await;
        assert!(matches!(decision, InterceptDecision::Allow));
    }

    #[tokio::test]
    async fn credential_in_headers_satisfies_gate() {
        let mut headers = HashMap::new();
        headers.insert(
            "Authorization".to_string(),
            Value::String("Bearer xyz".to_string()),
        );
        let mut args = HashMap::new();
        args.insert("headers".to_string(), Value::Object(headers));
        let decision = gate(true).pre_invoke("slack.post", &args).await;
        assert!(matches!(decision, InterceptDecision::Allow));
    }

    #[tokio::test]
    async fn non_auth_tool_is_untouched() {
        let decision = gate(true).pre_invoke("read", &HashMap::new()).await;
        assert!(matches!(decision, InterceptDecision::Allow));
    }
}
