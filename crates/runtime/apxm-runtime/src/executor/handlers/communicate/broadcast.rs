...
mod tests {
    ...
    fn echo_dag(value: &str) -> ExecutionDag {
    ...
    #[tokio::test]
    async fn broadcast_fans_out_to_all_registered_agents() {
        let memory = Arc::new(
        ...
        flow_registry.register_flow("AgentA", "communicate", echo_dag("from-A"));
        flow_registry.register_flow("AgentB", "communicate", echo_dag("from-B"));
        flow_registry.register_flow("AgentC", "main", echo_dag("from-C"));
        ...
        assert_eq!(arr.len(), 3, "all three agents should respond");
        ...
        let strings: Vec<String> = arr
            ...
            .collect();
        assert!(strings.contains(&"from-A".to_string()));
        assert!(strings.contains(&"from-B".to_string()));
        assert!(strings.contains(&"from-C".to_string()));
    }

    #[tokio::test]
    async fn broadcast_returns_empty_array_when_no_agents_registered() {
        let memory = Arc::new(
            MemorySystem::new(MemoryConfig::in_memory_ltm())
                .await
                .unwrap(),
