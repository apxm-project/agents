...
mod tests {
    ...
    impl ParkAdmission for Counter {
        fn on_park(&self) {
    ...
    #[test]
    fn park_unpark_route_to_registered_handle() {
        let c = Arc::new(Counter {
            parks: AtomicUsize::new(0),
            unparks: AtomicUsize::new(0),
        });
        register("exec-adm-1".into(), c.clone());
        on_park("exec-adm-1");
        on_unpark("exec-adm-1");
        on_park("exec-adm-1");
        assert_eq!(c.parks.load(Ordering::SeqCst), 2);
        assert_eq!(c.unparks.load(Ordering::SeqCst), 1);
        unregister("exec-adm-1");
        // After unregister, calls are no-ops.
        on_park("exec-adm-1");
        assert_eq!(c.parks.load(Ordering::SeqCst), 2);
    }
    ...
    fn unknown_id_is_a_noop() {
        on_park("never-registered-xyz");
        on_unpark("never-registered-xyz");
    }
}
