...
mod tests {
    ...
    fn test_closed_by_default() {
        ...
        assert!(reg.is_available("backend"));
        assert_eq!(reg.health("backend").unwrap().state, CircuitState::Closed);
    }
    ...
    fn test_trips_after_threshold() {
        ...
        reg.register("backend");
        for _ in 0..3 {
            reg.record_failure("backend");
        }
        assert!(!reg.is_available("backend"));
        assert_eq!(reg.health("backend").unwrap().state, CircuitState::Open);
    }
    ...
    fn test_half_open_after_timeout() {
        ...
        // Should now be HalfOpen → available
        assert!(reg.is_available("backend"));
        assert_eq!(reg.health("backend").unwrap().state, CircuitState::HalfOpen);
    }

    #[test]
    fn test_closes_after_probe_success() {
        let reg = CircuitBreakerRegistry::new(fast_config());
        reg.register("backend");
        for _ in 0..3 {
            reg.record_failure("backend");
    ...
    fn test_success_rate() {
        ...
        reg.record_success("backend");
        reg.record_failure("backend");
        let h = reg.health("backend").unwrap();
        assert!((h.success_rate - 2.0 / 3.0).abs() < 1e-9);
    }

    #[test]
    fn test_unknown_backend_assumed_available() {
