...
impl ReadAuthorizationBinding {
    ...
    pub fn from_env() -> Result<Option<Self>, String> {
        ...
        let correlation = std::env::var(CORRELATION).ok();
...
impl CommitLocalStore {
    ...
    pub fn commit(
        ...
        let (evidence_records, observations) =
            self.prepare_commit_records(request, &evidence_position_ref)?;
...
fn validate_observation_batch(
    ...
    for observation in observations {
        ...
        if observation
            .output_ref
            .as_ref()
            .is_some_and(|reference| !output_refs.contains(reference.as_str()))
        {
            return Err(CommitLocalError::InvalidRequest(
