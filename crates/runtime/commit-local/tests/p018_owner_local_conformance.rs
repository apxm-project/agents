...
async fn node_inspection_is_invocation_bound_and_has_no_synthetic_air_node() {
    ...
    let prepared = port
        .prepare_output(output_preparation(
            "commit.node-inspection",
            "instance.node",
            "invoke.node",
        ))
        .expect("prepare node output");
    ...
    let mut resumed = observation_value("invoke.node", 3);
    resumed["node_execution_id"] = json!("node.execution");
    resumed["observation_kind"] = json!("event_resumed");
    resumed["event_ref"] = json!({"event_ref": "event.node"});
    ...
    commit.tuple.observations = vec![started, waiting, resumed, committed];
    bind_observation_digest(&mut commit);
    assert!(matches!(
        port.commit(commit).await,
        ExecutionCommitResult::Committed { .. }
