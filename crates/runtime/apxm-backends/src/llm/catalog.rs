...
mod tests {
    ...
    #[test]
    fn each_provider_has_at_most_one_default_model() {
        let providers: Vec<ProviderProtocol> = BUILTIN_MODELS
            .iter()
        ...
        for protocol in providers {
            let defaults: Vec<_> = BUILTIN_MODELS
                .iter()
                .filter(|model| model.protocol == protocol && model.is_default)
                .collect();
            assert!(
                defaults.len() <= 1,
                "Protocol '{protocol}' should have at most one default model"
            );
        }
    }

    #[test]
    fn models_have_unique_ids() {
