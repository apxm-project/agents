//! Pure per-session ledger semantics: per-tool call budgets.
//!
//! It owns only the semantics — budget enforcement over in-memory counters —
//! and holds no global registry and no durable store. Durability is a separate
//! concern behind [`SessionLedgerStore`], which a lifecycle/persistence owner
//! implements; the runtime plane keeps only the fail-closed charging rules.

use std::collections::HashMap;
use std::sync::Mutex;

/// Typed session-ledger failures. Charging fails closed: a tool call that
/// would exceed its budget is refused rather than served.
...
/// Per-session tool budgets over in-memory counters. Budgets are fixed at
/// construction; counters advance only through the fail-closed charge method.
/// This type is pure: it does not persist itself. A durable owner reads
...
impl SessionLedger {
    ...
    #[must_use]
    pub fn new(tool_budgets: HashMap<String, usize>) -> Self {
        Self {
            tool_budgets,
    ...
    #[must_use]
    pub fn with_state(tool_budgets: HashMap<String, usize>, state: LedgerState) -> Self {
        Self {
            tool_budgets,
