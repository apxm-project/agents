...
mod tests {
    ...
    #[test]
    fn task_builder() {
        let t = Task::new(1, "research", "Research a topic")
            .with_priority(10)
        ...
        assert_eq!(t.metadata.priority, 10);
        assert_eq!(t.depends_on, vec![0]);
    }

    #[test]
    fn task_dag_validates_missing_dependency() {
        let dag = TaskDag::new("test").add_task(Task::new(1, "a", "A").add_dependency(99));

        let err = dag.validate().expect_err("should detect missing dep");
        assert!(err.to_string().contains("unknown task id=99"));
    }

    #[test]
    fn task_dag_validates_duplicate_ids() {
        let dag = TaskDag::new("test")
            .add_task(Task::new(1, "a", "A"))
            .add_task(Task::new(1, "b", "B"));
    ...
    #[test]
    fn task_dag_validates_cycles() {
        let dag = TaskDag::new("test")
            .add_task(Task::new(1, "a", "A").add_dependency(2))
            .add_task(Task::new(2, "b", "B").add_dependency(1));
    ...
    #[test]
    fn task_dag_validates_success() {
        let dag = TaskDag::new("test")
            .add_task(Task::new(1, "a", "A"))
            .add_task(Task::new(2, "b", "B").add_dependency(1))
            .add_task(Task::new(3, "c", "C").add_dependency(2));

        dag.validate().expect("valid DAG should pass");
    }

    #[test]
    fn task_dag_to_execution_dag() {
        let dag = TaskDag::new("research-pipeline")
            .add_task(Task::new(1, "research", "Research a topic"))
            .add_task(Task::new(2, "fact-check", "Verify claims").add_dependency(1))
            .add_task(
                Task::new(3, "write", "Write report")
                    .add_dependency(1)
                    .add_dependency(2),
            );
        ...
        // 3 synthetic nodes (one per task)
    ...
    #[test]
    fn task_dag_parallel_tasks() {
        // Two independent tasks with no deps should both be entry nodes
        let dag = TaskDag::new("parallel")
            .add_task(Task::new(1, "a", "Task A"))
            .add_task(Task::new(2, "b", "Task B"));
    ...
    #[test]
    fn task_dag_json_roundtrip() {
        let dag = TaskDag::new("json-test")
            .add_task(Task::new(1, "a", "Task A"))
            .add_task(Task::new(2, "b", "Task B").add_dependency(1));

        let json = dag.to_json().expect("serialize task dag");
        let restored = TaskDag::from_json(&json).expect("deserialize task dag");
        assert_eq!(restored.name, "json-test");
        assert_eq!(restored.tasks.len(), 2);
    }

    #[test]
    fn task_source_id_stamped_on_nodes() {
        let dag = TaskDag::new("source-id").add_task(Task::new(10, "a", "Task A"));
        let exec_dag = dag.to_execution_dag().expect("should lower");
        assert_eq!(exec_dag.nodes.len(), 1);
        assert_eq!(exec_dag.nodes[0].metadata.task_source_id, Some(10));
    }
}
