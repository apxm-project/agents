...
mod tests {
    ...
    fn wait_for_lines(path: &std::path::Path, expected_len: usize) -> Vec<String> {
        for _ in 0..50 {
            if let Ok(contents) = fs::read_to_string(path) {
    ...
    #[test]
    fn every_hook_event_kind_renders_event_field() {
        let temp = tempdir().unwrap();
        let output = temp.path().join("hook-events.out");
        let hooks = HookEvent::ALL
            .into_iter()
            ...
            .collect();
        let emitter = SubprocessHookEmitter::new(hooks);

        emitter.emit_graph_start("exec-1", 2);
        emitter.emit_graph_end("exec-1", 2, true);
        ...
        emitter.emit_tool_start("mock-tool", &HashMap::new());
        emitter.emit_tool_end("mock-tool", &Value::String("ok".to_string()));

        let mut actual = wait_for_lines(&output, HookEvent::ALL.len());
        actual.sort();
        let mut expected: Vec<String> = HookEvent::ALL
            .into_iter()
    ...
    #[test]
    fn node_complete_hook_renders_operation_end_fields() {
        let temp = tempdir().unwrap();
        let output = temp.path().join("hook.out");
        let emitter = SubprocessHookEmitter::new(vec![HookConfig {
            event: HookEvent::NodeComplete,
            command: format!(
                "printf '%s|%s|%s|%s|%s|%s|%s|%s' \
                 {{{{event}}}} {{{{node_id}}}} {{{{op_type}}}} {{{{duration_ms}}}} \
                 {{{{success}}}} {{{{input_tokens}}}} {{{{prefill_ms}}}} {{{{decode_ms}}}} > {}",
                output.display()
            ),
            shell: None,
        }]);
        ...
        assert_eq!(wait_for_file(&output), "node_complete|42|ASK|7|true|3|2|4");
    }
