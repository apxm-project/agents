...
//!
//! On a synth-style fixture (multi-input downstream node consuming
//! `{ask_a}/{ask_b}/{ask_c}`) the runtime separately rejects the synth node
//! at both O1 and O2 with "template references unknown placeholder
//! '{ask_a}': not in input_names", suggesting the AIR\u2192MLIR\u2192wire
//! lowering loses or renames named upstream references for downstream
//! ASK consumers. Out of scope for Phase A; tracked as a follow-up.
