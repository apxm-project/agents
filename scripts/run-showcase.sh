...
print_info "Running: dekk apxm compile showcase.apxm -O0"
echo

if dekk apxm compile "$SHOWCASE_DIR/showcase.apxm" -O0 -o "$OUTPUT_DIR/showcase_O0.apxmobj" 2>&1 | tee "$OUTPUT_DIR/compile_O0.log"; then
    ...
    print_warning "O0 compilation failed (check if .apxm was generated)"
fi
echo

# Step 3: Compile at O2
print_step "Step 3: Compile at O2 (all optimizations)"
print_info "Running: dekk apxm compile showcase.apxm -O2 --emit-diagnostics"
echo

if dekk apxm compile "$SHOWCASE_DIR/showcase.apxm" -O2 -o "$OUTPUT_DIR/showcase_O2.apxmobj" --emit-diagnostics "$OUTPUT_DIR/diagnostics_O2.json" 2>&1 | tee "$OUTPUT_DIR/compile_O2.log"; then
    ...
    print_warning "O2 compilation failed (check if .apxm was generated)"
fi
echo

# Step 4: Analyze parallelism
print_step "Step 4: Analyze parallelism and critical path"
print_info "Running: dekk apxm analyze showcase.apxm"
echo

if dekk apxm analyze "$SHOWCASE_DIR/showcase.apxm" --json > "$OUTPUT_DIR/analysis.json" 2>&1; then
    ...
    print_warning "Analysis failed (check if .apxm was generated)"
fi
echo
...
if [ -f "$OUTPUT_DIR/showcase_O0.apxmobj" ]; then
    ...
    if dekk apxm decompile "$OUTPUT_DIR/showcase_O0.apxmobj" > "$OUTPUT_DIR/decompiled_O0.apxm" 2>&1; then
        print_success "Decompiled O0: $OUTPUT_DIR/decompiled_O0.apxm"
    fi
fi

if [ -f "$OUTPUT_DIR/showcase_O2.apxmobj" ]; then
    print_info "Decompiling O2 artifact..."
    if dekk apxm decompile "$OUTPUT_DIR/showcase_O2.apxmobj" > "$OUTPUT_DIR/decompiled_O2.apxm" 2>&1; then
        print_success "Decompiled O2: $OUTPUT_DIR/decompiled_O2.apxm"
    fi
fi
echo
...
if dekk apxm explain "$SHOWCASE_DIR/showcase.apxm" > "$OUTPUT_DIR/explanation.txt" 2>&1; then
...
echo "  2. Compare artifacts:   diff $OUTPUT_DIR/decompiled_O0.apxm $OUTPUT_DIR/decompiled_O2.apxm"
...
echo "  dekk apxm execute $SHOWCASE_DIR/showcase.apxm --emit-session --emit-metrics $OUTPUT_DIR/metrics.json"
echo
