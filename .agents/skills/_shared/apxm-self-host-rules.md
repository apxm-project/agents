...
1. **No raw build tooling in agent prompts.** Never instruct an agent to
   ...
   `dekk apxm <command>`, the same path a human uses.
...
3. **No hardcoded provider profiles.** Do not name `claude`, `codex`, or
   ...
   `reviewer`) and bind providers at the edge — `apxm-goal-orchestrator`
...
- Dispatch with `dekk apxm execute <workflow.py>`; obey
