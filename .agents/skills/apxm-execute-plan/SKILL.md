...
5. **Surface blockers immediately**. If a verification fails, a
   ...
   stop, summarize, and re-invoke `apxm-plan` rather than working
   around the blocker.
6. **Honor the AIS codegen contract**. After any `.td` or
   TableGen-shim edit, run `dekk apxm build-dialect` then
   `dekk apxm codegen` *before* the next phase begins.
...
- Touched a `crates/<x>` source? `dekk apxm test -p <x>`.
- Touched the CLI? `dekk apxm test-cli`.
- Touched Python frontend? `dekk apxm test-python-frontend`.
